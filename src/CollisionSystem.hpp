	...
	for (auto entity1 : view)
	{
		...
		for (auto entity2 : view)
		{
			...
			else if (registry.all_of<RigidBodyComponent>(entity1) && registry.all_of<RigidBodyComponent>(entity2))
			{
				...
				glm::vec3 normal = glm::normalize(c2.boundingSphere.center - c1.boundingSphere.center);
				c1.boundingSphere.center -= normal * (penetration * 0.5f);
				c2.boundingSphere.center += normal * (penetration * 0.5f);
